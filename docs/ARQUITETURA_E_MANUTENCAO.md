...
│   ├── models/                # Modelos SQLAlchemy + enums.py (fonte de verdade das tabelas; 020: + backup_records; 021: + backup_config; 030: + notification[EmailConfig, Notification]; 031: + onedoc_integration[OneDocIntegration]; 045: + backup_external_config, backup_external_record)
...
| `backup_external_config` | `BackupExternalConfig` (feature 045) | singleton `id=1` (lazy via `create_all`); `enabled` (bool, default **False** — cópia externa desativada), `dest_type` ("PASTA_REDE"), `dest_path` (caminho montado no SO — sem credenciais; None = não configurado); rastreio `updated_at`/`updated_by` |
| `backup_external_records` | `BackupExternalRecord` (feature 045) | resultado FINAL por backup: `filename` unique+índice (mesmo nome do local); `backup_type`; `status` SUCCESS/FAILURE; `copied_at` (UTC) índice; `size_bytes`/`sha256` (SUCCESS); `error_description` (motivo controlado — nunca segredos); sem FK física (vínculo lógico por `filename` com `backup_records`); nenhuma rotina edita/apaga (histórico imutável) e nenhuma rotina varre o destino (sem retenção externa) |
...
| Configuração do backup (021/022) | `app/services/backup_config_service.py` (efetiva: persistido → env → default; validação e salvamento; parâmetro `create=False` para leitura sem efeito colateral — 022), `app/models/backup_config.py` (singleton), rotas `GET/POST /admin/backups/configuracoes` (`admin_routes.py`; POST redireciona para `/admin/backups?success=|error=`), acesso pela interface pelo botão ⚙ no header da página de Backups (modal `#modalBackupConfig` em `templates/admin/backups.html` — 022), evento `BACKUP_CONFIGURACAO_ALTERADA` (before/after), snapshot renovado por tick no `backup_scheduler` (aplicação sem reinício; **fallback de boot**: em falha de leitura o scheduler mantém o snapshot anterior ou usa env/default se ainda não houver snapshot) |
| Destino externo de backups (045) | `app/services/external_backup_service.py` (cópia atômica tmp oculto + fsync + SHA-256 + `os.replace`; retry 3×/2s com orçamento total 120 s — constantes `EXTERNAL_COPY_*` monkeypatcháveis; teste de destino §24; registro final único por `filename`), `app/models/backup_external_config.py` + `app/models/backup_external_record.py`, gancho único em `BackupService.generate_backup` (chamada `process_backup_after_success` imediatamente após `_record_backup_success`; nunca propaga exceção), rotas `POST /admin/backups/externo/testar` + extensão do POST `/admin/backups/configuracoes` + flash composto local+externo no POST `/admin/backups/gerar` (`admin_routes.py`; gate `backup.gerenciar`), fieldset "Backup externo" no modal de configurações, coluna "Externo" no histórico e card "Destino externo" em `templates/admin/backups.html`; eventos `BACKUP_DESTINO_EXTERNO_CONFIGURADO`, `BACKUP_DESTINO_EXTERNO_TESTADO`, `BACKUP_EXTERNO_SUCESSO`, `BACKUP_EXTERNO_FALHA`; default DESABILITADO (comportamento atual preservado); destino nunca varrido (sem retenção externa) |
