...
- **FR-005**: A barra de pesquisa, o botão "Ler QR", a tabela e todos os elementos abaixo dela (mensagens, estados, avisos, dicas) MUST ocupar exatamente a mesma largura disponível do container do card — nenhuma coluna ou bloco começa/termina em posição diferente do bloco superior.
...
- **FR-008**: O botão "Ler QR" MUST permanecer integralmente visível em qualquer largura de viewport — nunca cortado, fora da tela ou sobreposto ao campo de pesquisa (reorganização vertical da linha pesquisa+botão é permitida em telas pequenas). O estilo de contorno atual do botão é preservado (decisão C-2) — nenhuma mudança de hierarquia visual.
