...
- **Colgroup real** no template (`inc-tag`/`inc-equip`/`inc-obs`) e span `.dash-ellip` + tooltip no **Equipamento** (substitui o `text-truncate` com `max-width:200px` — antes: largura travada em ~200px com corte sem consulta; depois: 44% da largura do card, cortando apenas quando excede, com tooltip).
...
- **Marcador do script atualizado**: em V0 injeta classe de referência `inc-table` + colgroup neutro; em V1 usa o markup real (âncora do slice no atributo `class=...inc-table"`, para não confundir com o seletor `.inc-table` do `<style>`).
