...
8. **Toolbar pós-feedback (celular)**: o grupo de botões à direita da seleção não tinha `flex-wrap` e transbordava o card em ~375px (dois botões ≈350px lado a lado) — primeiro mitigado com `flex-wrap` e, a seguir, **reorganizado a pedido do solicitante**: "Voltar para seleção" movido para a linha dos controles (ao lado de "Limpar seleção") e "Imprimir etiquetas" em linha própria abaixo (`d-flex flex-column gap-2` no container; `flex-wrap` mantido na linha superior). IDs funcionais do JS (`sel-count`, `select-all-page`, `clear-selection`, `print-selected`) intocados. Ajuste apresentacional isolado, fora da tabela.
