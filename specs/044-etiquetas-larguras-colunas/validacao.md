...
8. **Toolbar pós-feedback (celular)**: o grupo de botões à direita da seleção não tinha `flex-wrap` e transbordava o card em ~375px (dois botões ≈350px lado a lado) — corrigido com `d-flex flex-wrap gap-2` (mesmo padrão da 040 nos headers); o grupo da esquerda já tinha `flex-wrap`. Ajuste apresentacional isolado, fora da tabela.
