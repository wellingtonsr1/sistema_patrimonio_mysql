...
- [X] T013 Rodar a suíte completa: `python -m pytest tests/ -q` — 100% verde incluindo os novos testes (SC-008/§37) e `git diff` coerente com o plan (nenhuma tabela existente alterada, nenhum asset estático tocado)
- [X] T014 Criar `specs/045-backup-destino-externo/validacao.md` (SC-009): resultado de cada cenário A–L, suíte final, verificação de auditoria (4 eventos, zero segredos) e o **relatório final obrigatório do §43** (arquivos alterados/novos, configuração, fluxos manual/automático, integridade, falhas, auditoria, testes, limitações reais — não inventar resultados)
- [X] T015 Atualizar `docs/ARQUITETURA_E_MANUTENCAO.md` (Princípio XI): novas tabelas, serviço, rotas na listagem de endpoints e comportamento do backup com destino externo; marcar tasks concluídas e commit do grupo lógico no padrão do repositório (subject "Feature 045: ..." sem acentos, footer Codebuff)
